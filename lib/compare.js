...
async function compareDatabases(sourceConfig, targetConfig, verbose) {

    await psql.analyse(credentials, { dbname: sourceConfig.dbname });
    ...
    for (const sourceDb of sourceDatabases) {
        ...
        idx++
        if (verbose) {
            console.log(`Processing database ${sourceDb} (${idx}/${sourceDatabases.length})`);
        }
...
function compare(config) {
    ...
    return compareDatabases(sourceConfig, targetConfig, verbose).then(function (discrepancies) {
        return discrepancies;
    });
}
